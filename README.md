...
```mermaid
flowchart LR
    Client[Client / App / Portal] --> API[API Gateway / Frontend Layer]
    API --> PCS[Provider Content Service]
    API --> ID[Identity Service]
    PCS --> DB[(MySQL Database)]
    ID --> DB2[(User / Profile Data)]
    PCS --> Config[Spring Cloud Config Server]
    PCS --> Eureka[Eureka Discovery]
```
...
For questions or collaboration related to this project, you can contact the repository owner or project maintainer through the GitHub profile associated with this repository.

---

Built with care for a modern home services platform and designed as part of a distributed systems master's project.
